...
function MediaRow({
  ...
  return (
    <div className="flex items-center gap-4 px-4 py-3 hover:bg-[var(--accent)]/50 transition-colors">
      <div className="relative h-10 w-10 rounded-lg bg-[var(--accent)] flex items-center justify-center shrink-0 overflow-hidden">
        {item.type === "IMAGE" ? (
          <Image src={item.url} alt="" fill className="rounded-lg object-cover" />
        ) : (
          getMediaIcon(item.type)
        )}
      </div>
